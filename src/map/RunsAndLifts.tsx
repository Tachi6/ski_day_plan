...
export const RunsAndLifts = () => {
  ...
  const { runs, lifts, status } = useObtainData(
    selectedResort?.dbName,
    selectedResort?.specialFilter,
  );
  ...
  useEffect(() => {
    if (!isPanesCreated.current) {
      ...
      map.getPane('lifts')!.style.zIndex = '401';
      map.getPane('runs')!.style.zIndex = '400';
